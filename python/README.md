...
> This is the **Python implementation** (`python/`). A parallel **pure-Julia + GPU** implementation
> lives alongside it in [`../julia/`](../julia/) — no C/C++ dependencies (the HiGHS MILP, the `ldpc`
> BP-OSD decoder, and `igraph` BLISS are reimplemented natively), with a CUDA kernel verified on an
> NVIDIA A100. It is cross-validated against this Python package (55 tests). See the
> [top-level README](../README.md) to choose between the two.
