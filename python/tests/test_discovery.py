...
# The paper catalog is NOT bundled with the released package (held-out test data, and the paper's
# own source). Tests that compare against the full catalog skip unless a local copy is present;
# the landmark-only validation tests (below) always run and need no external data.
PBB_CATALOG = Path(__file__).resolve().parent.parent / "ref-paper" / "arxiv-2606.02418" / "src" / "pbb_catalog_tables.tex"
needs_catalog = pytest.mark.skipif(not PBB_CATALOG.exists(),
                                   reason="paper catalog not bundled (supply ref-paper/ to run)")
