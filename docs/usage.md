...
- **Discovery is catalog-blind**: it never reads the paper catalog. `qcode-validate` is the only
  catalog reader and runs *after* discovery. Every discovered `[[n,k,d]]` is logged with its
  verifier status to `results/discovery_timeline.md` before any paper comparison.

## Claude-as-LLM guided search

`scripts/run_claude_guided_search.py --gen <label> --propose '<ansatz-json>'` evaluates a
Claude-proposed generator ansatz (blind) and records the lineage; see `docs/REPORT.md`.

## Tests & audit

```bash
python -m pytest -q          # 49 passing (kernel + discovery + theorem witnesses)
