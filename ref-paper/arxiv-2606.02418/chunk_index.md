...
|---|---|---|---|---|---|---|---|
| chunk_001 | `src/paper.tex` | l.60-770 | pp.1-16 | Main text: `sec:intro` (61), `sec:related` (108), `sec:prelim` (151) incl. `sec:bb` (155) / `sec:pbb` (171) / `sec:fom` (199) / `sec:ab_trap` (206); `sec:method` (214) incl. `sec:framework` (218) / `sec:cascade` (244) / `sec:campaigns` (261); `sec:verification` (303) incl. `sec:bposd_limits` (307) / `sec:milp` (320) / `sec:noncss_pipeline` (339) / `sec:trust` (367); `sec:results` (374) incl. `sec:families` (378) / `sec:tradeoff` (534) / `sec:comparison` (563) / `sec:code_capacity` (633) / `sec:bposd_findings` (671) / `sec:ablation` (704); `sec:discussion` (724); `sec:conclusion` (754) | 35 labels: 25 `sec:*` (8 `\section` + 17 `\subsection`), 5 `tab:*` (`tab:campaigns`, `tab:pbb_best`, `tab:codes`, `tab:comparison`, `tab:useful`), 4 `fig:*` (`fig:pipeline`, `fig:pareto`, `fig:ler_curves`, `fig:tightening`), 1 `fn:depolarizing-decoder`. No `thm`/`lem`/`eq` labels in this range (statements forward-referenced). | `digest/chunk_001_main_pp.md` | DIGESTED |
